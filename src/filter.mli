...
(** [f1 >>> f2] composes two filters sequentially.
    ...
    The request flows through [f2] first, then [f1]. The
    response flows back through [f1], then [f2]. This matches
    the intuitive left-to-right reading order.

    Example:
    {[
      let combined =
        strip_prefix ~prefix:"/api"
        >>> logging_filter >>> auth_filter
...
val apply_all :
  ...
  -> ('req, 'res) Service.t
  -> ('req, 'res) Service.t
(** [apply_all filters service] applies a list of filters to a
    service.

    Filters are applied in order from first to last, meaning
    the first filter in the list is the outermost filter
...
(** [strip_prefix ~prefix] creates a filter that removes a path
    prefix from incoming requests.

    This is useful for mounting services at specific paths,
    which is the case for serving static files *)
